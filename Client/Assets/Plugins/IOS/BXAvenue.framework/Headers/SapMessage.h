...
typedef struct stForwardMsg
{
	unsigned int dwComposeSrvId;
	unsigned int dwComposeMsgId;
	unsigned int dwOriginalSeq;
	char pBuffer[0];	
}SForwardMsg;
...
public:
	...
	void SetIdentifier(unsigned char byIdentifer);
	void SetServiceId(unsigned int dwServiceId);
	void SetMsgId(unsigned int dwMsgId);
	void SetCode(unsigned int dwCode);
