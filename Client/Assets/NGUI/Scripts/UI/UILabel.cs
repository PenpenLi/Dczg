...
public class UILabel : UIWidget
{
	...
	[HideInInspector][SerializeField] int mSpacingY = 0;	// add by xuzheng
		...
		if (mPrintedSize > 0)
		{
			for (;;)
			{
				...
				if (!string.IsNullOrEmpty(mProcessedText))
				{
                    if (mFont != null)
                    {
                        Vector3 size = mFont.CalculatePrintedSize(mProcessedText, fs, mEncoding, mSymbols);
                        mCalculatedSize = new Vector3(size.x, size.y + size.z * spacingY);
                    }
#if DYNAMIC_FONT
                    else
                    {
                        Vector3 size = NGUIText.CalculatePrintedSize(mProcessedText, mTrueTypeFont, fs, mFontStyle, mEncoding);
                        mCalculatedSize = new Vector3(size.x, size.y + size.z*spacingY);
                    }
#endif
				}
