...
public class UIRoot : MonoBehaviour
{
	...
	// 设定期望的目标分辨率，如果OriginWidth>0，则优先保证宽度，即在屏幕上下留黑边 by 徐峥
	...
	protected virtual void Start ()
	{
		...
		// OriginWidth>0，则优先保证宽度，即在屏幕上下留黑边 by 徐峥
		if( OriginHeight > 0 )
			manualHeight = OriginHeight;
		if( (scalingStyle==Scaling.FixedSize || scalingStyle==Scaling.FixedSizeOnMobiles) && (OriginWidth>0) && (OriginHeight>0) )
		{
			float fRadio = ((float)OriginWidth/(float)OriginHeight) / ((float)Screen.width/(float)Screen.height);
			if( fRadio > 1f ) 
				manualHeight = (int)(OriginHeight * fRadio);
		}
	}
