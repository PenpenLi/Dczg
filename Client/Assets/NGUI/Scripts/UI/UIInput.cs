...
#else
    protected int cursorPosition { get { return (isSelected && mEditor != null) ? mEditor_SelectPos() : value.Length; } }
#endif

    private int mEditor_SelectPos()
    {
        return mEditor.selectIndex;
    }
	...
	bool ProcessEvent (Event ev)
	{
		...
		switch (ev.keyCode)
		{
			...
			case KeyCode.KeypadEnter:
			{
				...
				{
					char c = '\n';
                    if (onValidate != null) c = onValidate(mEditor.content.text, mEditor_SelectPos(), c);
                    else if (validation != Validation.None) c = Validate(mEditor.content.text, mEditor_SelectPos(), c);
