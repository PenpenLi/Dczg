...
public class sdUIPetLevelSelectCard : MonoBehaviour 
{
		...
		if (m_uuDBID!=UInt64.MaxValue)
		{
			...
			if (wnd)
			{
				...
				if (petPnl)
				{
					if (m_bSelect==true)
					{
						petPnl.RemoveSelectDBID(m_uuDBID);
						petPnl.ReflashPetIconSelectUI();
					}
					else
					{
						int iSelectCount = petPnl.GetSelectIconNum();
						if (iSelectCount<8)
						{
							petPnl.SetSelectDBID(m_uuDBID);
							petPnl.ReflashPetIconSelectUI();
						}
					}

					petPnl.OnClickSelectSetGray();
				}
			}
		}
	}
	...
	public void ReflashGrayUI(bool bGray)
	{
		if (bGray)
		{
