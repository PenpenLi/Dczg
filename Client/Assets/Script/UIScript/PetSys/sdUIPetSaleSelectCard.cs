...
public class sdUIPetSaleSelectCard : MonoBehaviour 
{
	...
	void OnClick()
	{
		if (gameObject.activeSelf==true && m_uuDBID!=UInt64.MaxValue && m_uuDBID!=0)
		{
			m_bSelect = !m_bSelect;
			ReflashSelectUI();
			...
			if (wnd)
			{
				sdUIPetSaleSelectPnl petPnl = wnd.GetComponentInChildren<sdUIPetSaleSelectPnl>();
				if (petPnl)
					petPnl.ReflashPetSaleMoneyUI();
			}
		}
	}
